...
from src.core.models import Columns
...
class Filter_Frame(tk.LabelFrame):
    ...
    def _build_ui(self):
        tk.Label(self, text="Введите столбец для фильтрации").grid(row=0, column=0, columnspan=2, padx=10, pady=5, sticky='w') # noqa E501

        self.entry_column = tk.Entry(self)
        self.entry_column.grid(row=1, column=0, padx=10, pady=5)
        ...
        tk.Label(self, text='Чтобы принять \nзначения нажмите "OK"').grid(row=4, column=0, sticky='w')
        tk.Button(self, text='Ok', command=self._get_filter_value).grid(row=4, column=1, padx=10, pady=5, sticky='w')

    def _get_filter_value(self):
        """ Сохраняем значения из поля ввода"""
        try:
            column = self.entry_column.get()
        except tk.TclError:
            box.showwarning("Ошибкаб", "Выберите колонку из списка!")
            return

        self.request.filter_column = column
        value = self.entry_filter.get()
