...
class ExcelProcessor:
    def process(self, request: ProcessingRequest) -> ProcessingResult:
        ...
        try:
            ...
            with self._load_workbook_context(request.source_path) as (wb, sheet):
                requered_columns = [col.value for col in Columns]
                header_info = self._get_index_filter_columns(sheet, request, requered_columns)
                
                filtered_data = self._filter_data(sheet, request, header_info, requered_columns)
    ...
    def _validate_request(self, request: ProcessingRequest) -> ProcessingResult:
        ...
        if not request.target_path:
            return ProcessingResult(
                success=False,
                message="Не указан путь для сохраненеия результат",
                output_path=None
            )
        ...
        if not request.source_path.lower().endswith('xlsx'):
            return ProcessingResult(
                success=False,
                message="Поддерживается только файлы Excel ('*.xlsx')",
                output_path=None
            )
    ...
    def _filter_data(self, sheet, request, header_info, requered_columns):
        
        try:
            with self._save_file_context(request.target_path) as (wb_out, sheet_out, start_row):
                self._format_header(sheet_out, start_row, requered_columns)
                ...
                for row_value in sheet.iter_rows(values_only=True, min_row = header_info[1] + 1 ):
                    if row_value[index_filter_column] == request.filter_item:
                        filtered_row = [row_value[header_info[0][col]] for col in requered_columns]
    ...
    def _get_index_filter_columns(self, sheet, request, requered_columns):
        """
        ...
        for row_index, row_value in enumerate(sheet.iter_rows(values_only=True), 1):
            ...
            requered_columns_set = set(requered_columns)
            # Ищем строку содержащюю столбец фильтрации
            if requered_columns_set.issubset(row_columns_set):
                print(row_columns_set, requered_columns_set)
                columns_index = {}
                # Собираем инедксы всех требуемых столбцов
                for column_name in requered_columns:
    ...
    def _load_workbook_context(self, source_path: str):
        ...
        try:
            ...
            if file_size > max_size:
                return ProcessingResult(
                    success=False,
                    message=f"Файл слишком большой {file_size / (1024*1024):.1f}MB (максимум 50MB)",
                    output_path=None
                ) 
            ...
            if not sheet:
                return ProcessingResult(
                    success=False,
                    message="В файле нет активного листа",
                    output_path=None
                )
